    ...
    <div class="py-12">
        <div class="container">
            ...
            <form class="w-50" action="{{ route('users.update', $user->id) }}" method="POST">
                ...
                @method('PUT')
                <div class="mt-2">
                    <input type="text" required placeholder="Name" name="name" class="form-control" value="{{ $user->name }}">
                    @error('name')
                    <div class="alert alert-danger mt-2">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mt-2">
                    <input type="email" required placeholder="E-mail" name="email" class="form-control" value="{{ $user->email }}">
                    @error('email')
                    <div class="alert alert-danger mt-2">{{ $message }}</div>
                    @enderror
                </div>
